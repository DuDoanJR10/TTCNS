...
const ModalReport = ({ open, handleClose, listExport, listImport, date }) => {
  ...
  const handleSupplies = (dataBill) => {
    ...
    const listItemSupplies = suppliesExport?.reduce((init, supply) => {
      const itemSupplies = suppliesExport?.filter(item => item.name === supply.name);
      suppliesExport = suppliesExport?.filter(item => item.name !== supply.name);
      init =[...init, itemSupplies]
      return init
    }, [])
    let listSuppliesExport = listItemSupplies?.filter(item => item.length > 0);
    listSuppliesExport = listSuppliesExport?.map(listItem => {
      return listItem.sort((left, right) => moment(left?.created) - moment(right?.created));
    })
  ...
  const handleData = (dataExport = [], dataImport = []) => {
    ...
    let listData = suppliesTotal?.map(listSupply => {
      ...
      if (listSupply[listSupply.length - 1].hasOwnProperty('quantityImport')) {
        end = listSupply[listSupply.length - 1]?.oldQuantity + listSupply[listSupply.length - 1]?.quantityImport;
      } else {
        end = listSupply[listSupply.length - 1]?.oldQuantity - listSupply[listSupply.length - 1]?.quantityExport;
      }
      ...
      return {
        start, end, importIn, exportIn, _id: name
      }
    })
    ...
    let listSuppliesData = listSupplies.map(supplies => {
      const supplyExist = listData.find(item => item?._id === supplies?._id);
      if (supplyExist) {
        return supplyExist;
      } else {
        return {
  ...
  const getName = (id) => listSupplies.find(supply => supply?._id === id)?.name;
  return (
    <Modal
      centered
      width={1200}
    ...
    >
      <div className="flex text-lg justify-center mb-3">
        <h2 className='mr-1'>{`TỪ NGÀY: ${moment(date?.fromDate).format('DD/MM/YYYY')}`}</h2>
        <h2>{`ĐẾN NGÀY: ${moment(date?.toDate).format('DD/MM/YYYY')}`}</h2>
      </div>
      <Table columns={columns} dataSource={handleData(listExport, listImport)}/>
    </Modal>
  );
};
